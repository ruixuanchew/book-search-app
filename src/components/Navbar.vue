<template>
  <nav
    class="bg-brand-primary text-brand-secondary px-6 py-4 flex justify-between items-center shadow-lg sticky top-0 z-50"
  >
    ...
    <ul class="hidden md:flex space-x-64 mx-auto">
      ...
      <li class="relative group">
        ...
        <ul
          class="absolute left-0 mt-2 w-52 bg-brand-accent text-black rounded shadow-lg opacity-0 group-hover:opacity-100 group-hover:translate-y-1 transform transition max-h-48 overflow-y-auto custom-scrollbar"
        >
          <li
            v-for="genre in genres"
            :key="genre"
            class="px-4 py-2 hover:bg-brand-hover cursor-pointer"
      ...
      <li class="relative group">
        ...
        <ul
          class="absolute left-0 mt-2 w-40 bg-brand-accent text-black rounded shadow-lg opacity-0 group-hover:opacity-100 group-hover:translate-y-1 transform transition max-h-48 overflow-y-auto custom-scrollbar"
        >
          <li
            v-for="lang in languages"
            :key="lang.code"
            class="px-4 py-2 hover:bg-brand-hover cursor-pointer"
             @click="$emit('select-language', lang.code)"
          >
            {{ lang.label }}
          </li>
        </ul>
...
export default {
  ...
  data() {
    return {
      ...
      viewabilities: ["ALL PAGES", "PARTIAL", "NO PAGES"]
    }
  },
}
</script>
